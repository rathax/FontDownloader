<template>
    <v-sheet class="h-full max-w-[960px] mx-auto py-10 px-10 leading-7 font-light">
     <p><i>Last updated: July 21, 2024</i></p>
     <h1 class="text-h3 mt-8 mb-6">
       Contact
     </h1>
     <p class="my-4">
        If you want to reach me, leave me an email at <a href= "mailto: rathaxdev@gmail.com" class="font-bold">rathaxdev@gmail.com</a>. I'm happy to help you with any questions or concerns you may have. Whether you need assistance with downloading fonts, have a suggestion, or need technical support, we're just a message away.
     </p>

     
   </v-sheet>
   </template>
 
 <script setup lang="ts">
useSeoMeta({
  creator: 'ratHax',
  author: 'ratHax',
  title: `FontDownloader | Contact`,
  ogTitle: `FontDownloader | Contact`,
  description: `Information on how to contact the author of FontDownloader.org`,
  ogDescription: `Information on how to contact the author of FontDownloader.org`,
  ogUrl: "https://fontdownloader.org/contact",
})
useHead({
   link: [{ rel: "canonical", href: "https://fontdownloader.org/contact"}],
})
 </script>